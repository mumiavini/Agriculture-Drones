...
\title{\textbf{PIBITI}\\[0.3em]Análise Comparativa de Motores e Hélices de uso em Aeronaves Agrícolas de Asa Fixa}
...
A transição das configurações multirrotor para arquiteturas de asa fixa apresenta-se como uma evolução técnica necessária para maximizar a cobertura de área por voo e otimizar a eficiência energética. Essa mudança justifica-se pela limitação dos multirrotores em satisfazer requisitos de larga escala, dada a sua dependência de sustentação vetorial contínua, que resulta em menor autonomia e menor área coberta por voo quando comparados às plataformas de asa fixa \cite{kartal2025}. Contudo, à medida que a plataforma escala em massa máxima de decolagem (MTOW), o problema deixa de ser apenas de seleção de componentes COTS (\textit{Commercial Off-The-Shelf}) e passa a se inserir em um regime de engenharia aeronáutica certificada, com requisitos termodinâmicos, estruturais e regulatórios substancialmente distintos dos VANTs convencionais.
...
\begin{itemize}
    ...
    \item \textbf{Aderência ao mercado brasileiro:} O EMB-202/203 Ipanema (MTOW \SI{1800}{kg}) é a plataforma líder da pulverização aérea brasileira, com cerca de 60\% de participação na frota nacional \cite{embraer_1700}, situando-se exatamente no centro da faixa estudada.
...
\begin{itemize}
    ...
    \item \textbf{Atmosfera operacional:} As missões ocorrem em ambientes com material particulado em suspensão e vapores químicos provenientes dos defensivos. Essa condição exige índices elevados de proteção IP (\textit{Ingress Protection}) para os componentes eletrônicos, sistemas eficientes de filtragem de ar e, em aeronaves tripuladas, selagem da cabine, como ocorre no Ipanema e no AgWagon.
    ...
    \item \textbf{Variação de Massa e Centro de Gravidade:} Durante a fase de pulverização, o peso da aeronave decresce substancialmente à medida que a carga útil é ejetada, alterando a posição do centro de gravidade (CG) e a carga alar (\textit{wing loading}). No Ipanema EMB-202, o hopper comporta até \SI{950}{L} de produto \cite{embraer202_wiki}. Como a massa depende da densidade do líquido aplicado, esse volume corresponde, por exemplo, a \SI{760}{kg} para uma densidade de \SI{0.8}{kg/L} e a \SI{950}{kg} para \SI{1.0}{kg/L}, isto é, de aproximadamente 42\% a 53\% do MTOW de \SI{1800}{kg}. No Pawnee Brave 375, o hopper de aproximadamente \SI{1080}{L} (38 ft$^3$) também representa parcela substancial da massa total, cuja quantificação requer a densidade do produto transportado \cite{piper_pa36}.
\end{itemize}   

\subsection{Metodologia}

A pesquisa foi conduzida por meio de revisão bibliográfica e documental. Foram consultados manuais de operação e fichas técnicas dos fabricantes, \textit{Type Certificate Data Sheets} da FAA, regulamentos e instruções da ANAC, normas técnicas e artigos científicos sobre propulsão, materiais de hélices, eletrificação e monitoramento de aeronaves. Para cada plataforma de referência, foram levantados MTOW, capacidade do hopper, configuração e potência do motor, massa seca, intervalo entre revisões gerais (TBO), tipo de hélice, consumo e requisitos de certificação, conforme a disponibilidade desses dados nas fontes citadas.

Os motores e as hélices foram comparados quanto à faixa de potência, relação potência/massa, consumo específico, adequação ao envelope agrícola, disponibilidade de suporte e situação de certificação. As alternativas elétrica e híbrida foram avaliadas por balanços preliminares de energia e massa e por resultados reportados na literatura. Por fim, as combinações recomendadas foram organizadas por faixa de MTOW e perfil de missão do AGRO-VANT. Quando não havia curva pública específica para determinado modelo, empregou-se uma estimativa por semelhança com motores da mesma família; essas extrapolações são identificadas no texto e deverão ser confirmadas nas etapas posteriores de dimensionamento e ensaio.

\section{Comparação de Motores na Faixa de Interesse}
...
\begin{itemize}
    ...
    \item \textbf{Refrigeração a Ar:} A ausência de circuito de líquido refrigerante simplifica a manutenção em campo, condizente com a operação descentralizada da aviação agrícola em pistas rurais. Exige, contudo, defletores do motor cuidadosamente dimensionados para garantir uniformidade térmica entre cilindros \cite{lycomingmanual, io360manual}.
    ...
    \item \textbf{Confiabilidade:} O intervalo entre revisões gerais (TBO) recomendado pelos fabricantes para a família é de 1.500 a 2.000 horas em serviço intermitente \cite{lycomingmanual}. Esse intervalo é adequado ao perfil de utilização típico das aeronaves agrícolas, considerando as safras sazonais e o uso intensivo durante a janela operacional (estimativa do autor: 200 a 400 h/ano em safra plena).
\end{itemize}
...
A migração para sistemas eletrônicos de injeção e ignição (FADEC, \textit{Full Authority Digital Engine Control}) é uma tendência emergente, exemplificada pela linha Lycoming iE2, que oferece controle eletrônico integral com sensores MAP (\textit{Manifold Absolute Pressure}) e IAT (\textit{Intake Air Temperature}). O FADEC ajusta automaticamente a razão estequiométrica da mistura combustível e ar, o avanço de ignição e a mistura para a densidade local do ar, eliminando a necessidade de ajuste manual de mistura pelo piloto. Para operações em densidade de altitude variável (planaltos brasileiros, com elevações de \SI{600}{m} a \SI{1100}{m}), essa correção automática pode proporcionar maior eficiência operacional ao manter o motor próximo do regime de mistura desejado (regime pobre/econômico em cruzeiro ou regime rico de alto desempenho em decolagem) sem intervenção manual do piloto.
...
A aeronave agrícola de referência no Brasil é o Embraer/Neiva Ipanema, com cerca de \num{1700} unidades entregues ao longo de mais de cinco décadas de produção e participação de mercado próxima a 60\% da frota brasileira de pulverização aérea \cite{embraer_1700}. Suas especificações técnicas servem como \textit{benchmark} para qualquer plataforma de Classe IV nacional. Registre-se que a versão atualmente em produção é o EMB-203 \cite{embraer_1700}; o EMB-202/202A, contudo, é adotado como referência técnica deste estudo por concentrar a frota em operação e por constituir a base da certificação a etanol discutida adiante.

\begin{itemize}
    \item \textbf{MTOW:} \SI{1800}{kg}
    \item \textbf{Capacidade volumétrica do hopper:} \SI{950}{L}
...
A família Lycoming O-540 / IO-540, certificada pela FAA sob os \textit{Type Certificates} E-295 (série O-540) e 1E4 (série IO-540) \cite{faa_tcds_e295, faa_tcds_1e4}, é o motor de pistão dominante em aplicações agrícolas tripuladas dessa classe \cite{lycomingmanual}. Trata-se de um motor de 6 cilindros horizontalmente opostos (boxer), refrigerado a ar, com taxa de compressão entre 7{,}2:1 e 8{,}5:1. O peso seco situa-se em torno de \SI{438}{lb} (\SI{198.7}{kg}) e o BSFC em regime de cruzeiro é de aproximadamente \SI{0.48}{lb/(hp \cdot h)} (\SI{292}{g/(kW \cdot h)}) \cite{lycomingmanual}.
...
A documentação técnica dos motores Lycoming fornece dados quantitativos detalhados em duas curvas fundamentais que se aplicam, com adaptações de escala, a toda a família de motores boxer da marca: a curva de Consumo de Combustível em Carga Parcial (\textit{Part Throttle Fuel Consumption}) e a curva de Desempenho ao Nível do Mar e em Altitude (\textit{Sea Level and Altitude Performance}). A análise a seguir toma como referência metodológica as Figuras 3-5 e 3-21 do manual oficial do IO-360 \cite{io360manual}, o motor de 200 HP que, embora não seja diretamente empregado em aeronaves agrícolas da faixa de interesse, compartilha a mesma família construtiva e curvas funcionalmente equivalentes (escalonadas para o número de cilindros e cilindrada) com os motores O-540/IO-540 e IO-720, dominantes na faixa de 1 a 2{,}5 t. Essa metodologia foi adotada por restrição de acesso público à edição equivalente do manual da série O-540 (curvas detalhadas de Part Throttle e altitude/performance individuais por sufixo de motor).
...
\begin{figure}[H]
\centering
\includegraphics[page=52, clip, trim={1.8cm 4.4cm 1.8cm 3.4cm}, width=0.78\textwidth]{figures/lycoming_manual_360.pdf}
\caption{Curva de Consumo de Combustível em Carga Parcial (\textit{Part Throttle Fuel Consumption}), Lycoming IO-360-A, -C, -D, -J, -K e AIO-360. Reproduzida da Figura 3-5 do manual do fabricante \cite{io360manual}.}
\label{fig:part_throttle}
\end{figure}

A Figura \ref{fig:part_throttle} apresenta duas famílias de curvas:

\begin{itemize}
    \item \textbf{Melhor potência (\textit{Best Power}):} Razão ar/combustível ligeiramente rica (em torno de 12{,}5:1), maximizando o torque entregue. Adequada às fases de decolagem, ganho de altitude e manobras de retorno em pulverização agrícola.
    \item \textbf{Mistura de Melhor Economia (\textit{Best Economy}):} Razão ar/combustível próxima à estequiométrica (cerca de 14{,}7:1) ou ligeiramente pobre. Adequada ao cruzeiro e a deslocamentos, reduzindo o consumo específico em até 15\% em relação à mistura de melhor potência \cite{io360manual}.
\end{itemize}

No IO-360 (\SI{200}{HP}), a curva indica consumo aproximado de \SI{85}{lb/h} a \SI{170}{HP} (85\% da potência nominal) em mistura \textit{best power}, e de \SI{75}{lb/h} no mesmo regime em mistura \textit{best economy}. Esses valores correspondem a um BSFC de aproximadamente \SI{0.50}{lb/(hp \cdot h)} e \SI{0.44}{lb/(hp \cdot h)}, respectivamente, equivalentes a \SI{304}{g/(kW \cdot h)} e \SI{268}{g/(kW \cdot h)} \cite{io360manual}.

\textbf{Extrapolação para a Família O-540 / IO-540 / IO-720:} Como estimativa preliminar, a topologia da curva foi considerada semelhante para os demais motores da família, com o eixo de potência reescalado. Para o IO-540-K1J5 (\SI{320}{HP} do Ipanema EMB-202A), estima-se que o consumo em cruzeiro a 75\% (\SI{240}{HP}) se situe em aproximadamente \SI{130}{lb/h} em \textit{best power} e \SI{115}{lb/h} em \textit{best economy}; para o IO-720-D1CD (\SI{400}{HP} do Pawnee Brave 375), estimam-se valores de aproximadamente \SI{170}{lb/h} e \SI{150}{lb/h}, respectivamente \cite{lycomingmanual, lycoming_io720}. Esses valores extrapolados devem ser confirmados nas curvas específicas de cada variante antes do dimensionamento definitivo.
...
\begin{figure}[H]
\centering
\includegraphics[page=68, clip, trim={1.5cm 5.3cm 1.5cm 3.2cm}, angle=-90, width=0.95\textwidth]{figures/lycoming_manual_360.pdf}
\caption{Curva de Desempenho ao Nível do Mar e em Altitude (\textit{Sea Level and Altitude Performance}), Lycoming IO-360-A, -C, -D, -J, -K e AIO-360. Reproduzida da Figura 3-21 do manual do fabricante \cite{io360manual}.}
\label{fig:altitude_performance}
\end{figure}

A Figura \ref{fig:altitude_performance} é dividida em duas regiões:
...
\addlinespace
Lycoming IO-540-K1D5 / K1G5 / K1J5 & 6 / 8{,}9 L & \shortstack{\SI{300}{HP}/\SI{320}{HP}\\@ 2700 RPM} & $\sim$\SI{200}{kg} & \SI{2000}{h} & Ipanema 201/202 (1{,}5--1{,}8 t), Brave 300 (2{,}0 t) \\
\addlinespace
Lycoming IO-720-D1CD & 8 / 11{,}8 L & \shortstack{\SI{375}{HP}--\\\SI{400}{HP}\\@ 2650 RPM} & $\sim$\SI{253}{kg} & 1.800--\SI{2000}{h} & PA-36 Brave 375/400 (2{,}2 t) \\ \bottomrule
\end{tabularx}
\par\footnotesize Compilação do autor a partir de \cite{lycomingmanual, io360manual, lycoming_io720, continental_io520, embraer202, piper_pa25, cessna_188, piper_pa36}.
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{TBO competitivo:} Todos os motores apresentam TBO igual ou superior a \SI{1700}{h}, com a maior parte da família Lycoming O-540 atingindo \SI{2000}{h} \cite{lycomingmanual}. Cálculo do autor: a uma utilização média anual estimada em 300 h/ano (consistente com a janela operacional sazonal típica de safra brasileira), esse TBO equivale a aproximadamente 6{,}5 anos de operação entre revisões gerais.
...
A inviabilidade da tração puramente elétrica para aeronaves agrícolas de 1 a 2{,}5 t é demonstrada quantitativamente pelo balanço de massa de baterias. Tomando como caso de referência o IO-540-K1J5 do EMB-202 Ipanema (\SI{320}{HP} $\approx$ \SI{239}{kW}) operando por 2 horas em regime de cruzeiro a 75\% da potência nominal (\SI{179}{kW}), e adotando como densidade de energia da bateria os mesmos valores limítrofes da literatura já apresentados na Seção \ref{sec:ice}: Li-Ion atual de até \SI{330}{Wh/kg} (limite superior dos sistemas comercialmente disponíveis \cite{battery_aviation_review, aerospace_america_batteries}) e baterias avançadas em estado sólido de até \SI{400}{Wh/kg} (fronteira ainda não realizada comercialmente em escala \cite{battery_aviation_review}), o cálculo conduzido pelo autor é o seguinte:

\begin{itemize}
    \item \textbf{Energia necessária:} $E = 179\,\si{kW} \times 2\,\si{h} = 358\,\si{kWh}$
    \item \textbf{Massa de baterias Li-Ion (\SI{330}{Wh/kg}):} $m_{bat} = 358\,\si{kWh} / 0{,}33\,\si{kWh/kg} \approx 1085\,\si{kg}$
    \item \textbf{Massa com baterias em estado sólido (\SI{400}{Wh/kg}):} $m_{bat} \approx 895\,\si{kg}$
\end{itemize}

Com MTOW de \SI{1800}{kg} e peso vazio aproximado de \SI{1100}{kg}, a margem disponível para carga útil + combustível + baterias é de cerca de \SI{700}{kg}. Mesmo no cenário otimista com baterias em estado sólido, a massa de baterias (cerca de \SI{895}{kg}) excederia sozinha toda a margem operacional, eliminando totalmente a carga útil agroquímica. Para o Piper Brave 375 (\SI{400}{HP}, MTOW \SI{2177}{kg}), o cálculo resulta em valores ainda mais desfavoráveis.
...
Nessa configuração, o motor a combustão aciona exclusivamente um gerador, que fornece energia ao sistema elétrico da aeronave, incluindo os motores responsáveis pelo acionamento da hélice.
...
Para aplicação no AGRO-VANT, os parâmetros do sistema propulsivo deverão ser transmitidos à Estação de Controle em Solo (GCS, \textit{Ground Control Station}), permitindo o acompanhamento de RPM, pressão de admissão, fluxo de combustível, temperaturas e pressões. A arquitetura também deverá permitir o envio dos comandos de potência e prever procedimentos de contingência em caso de perda do enlace.
...
\begin{itemize}
    ...
    \item \textbf{Aviônica de Cabine:} Os dados de motor e pulverização podem ser apresentados por Sistemas Eletrônicos de Instrumentos de Voo (EFIS), como o Garmin G600 TXi, integrados a sistemas de navegação como o Avidyne IFD-540 e aos equipamentos de georreferenciamento agrícola. Entretanto, alguns exemplares de aeronaves clássicas (Pawnee, AgWagon, Ipanema EMB-201) ainda utilizam instrumentação analógica, o que dificulta a integração com sistemas de comunicação modernos e com arquiteturas de operação não tripulada.
\end{itemize}
...
A integração de \textit{Digital Twins} surge como tecnologia habilitadora para monitoramento em tempo real, manutenção preditiva e simulação de condições de contorno realistas. Para o AGRO-VANT, propõe-se a estruturação de um \textit{digital twin} composto por três camadas: (i) um modelo termodinâmico do conjunto motopropulsor calibrado por dados de bancada e pelas curvas de \textit{Part Throttle Fuel Consumption} apresentadas na Seção \ref{sec:curvas_lycoming}; (ii) um modelo multicorpo da estrutura da aeronave; e (iii) um modelo de degradação de componentes (fadiga acumulada na hélice e desgaste de cilindros) alimentado pela telemetria embarcada. Essa abordagem está alinhada com práticas emergentes da indústria aeronáutica, incluindo os sistemas de gerenciamento de saúde de frota da família Embraer E2 e os sistemas dos fabricantes de motores para frotas em operação \cite{sadeghi2024, airinsight_dt}. A abordagem também pode subsidiar políticas de manutenção baseada em condição (CBM, \textit{Condition-Based Maintenance}). A quantificação de ganhos específicos (por exemplo, a extensão do TBO efetivo) depende de validação experimental ainda não conduzida no escopo deste estudo e constitui uma linha de pesquisa para os próximos estágios do projeto.
...
\begin{itemize}
    ...
    \item \textbf{RBAC 33:} Regulamento de certificação de motores aeronáuticos a pistão \cite{anacrbac33}. Todos os motores Lycoming O-540, IO-540 e IO-720, bem como os Continental IO-520 e TSIO-520 mencionados neste trabalho, possuem certificações da FAA sob o predecessor CAR 13 ou equivalente ao RBAC 33, com type certificates específicos (e.g., TC 1E4 para a família IO-540 \cite{faa_tcds_1e4} e TC E-295 para a família O-540 \cite{faa_tcds_e295}).
    
    \item \textbf{RBAC 35:} Regulamento de certificação de hélices \cite{anacrbac35}. As hélices Hartzell e McCauley utilizadas nas aeronaves de referência possuem type certificates próprios (e.g., a família Hartzell HC-C2YK, da série Compact, sob o TC P-920 \cite{faa_tcds_p920}).
    
    \item \textbf{RBAC 137, Emenda nº 05 (Resolução nº 716/2023):} Em vigor desde 2 de outubro de 2023, estabelece o cadastro e os requisitos operacionais para operações aeroagrícolas no Brasil \cite{anac137}. A principal mudança da Emenda 05 foi a substituição do antigo Certificado de Operador Aéreo (COA) pelo Cadastro de Operador Aeroagrícola (CDAG), simplificando o processo de habilitação. Segundo o levantamento anual do Sindag, a frota aeroagrícola brasileira fechou 2025 com aproximadamente \num{2866} aeronaves, em sua maioria a pistão e com forte presença de Ipanemas e variantes \cite{sindag_frota2025}.
    ...
    \item \textbf{Normas técnicas complementares:} A ABNT NBR ISO 9001 é referência para sistemas de gestão da qualidade em fabricantes \cite{abnt9001}. A segurança operacional da atividade aeroagrícola é disciplinada, em complemento ao RBAC 137, pelas normas de trabalho da aviação agrícola do MAPA (Instrução Normativa nº 2/2008, com alterações da IN nº 37/2020), editadas com base no Decreto-Lei nº 917/1969 \cite{mapa_in02}. Para aeronaves não tripuladas, a ABNT publicou a NBR 17146 (procedimentos operacionais de UAS, alinhada à série ISO 21384 do ISO/TC 20/SC 16) \cite{abnt17146}. Para o segmento tripulado tradicional da faixa, a referência normativa principal permanece sendo os RBACs.
    
    \item \textbf{FAR Part 137 (FAA) e CS-VLA / CS-23 (EASA):} Regulamentos internacionais análogos, relevantes para os processos de validação de certificações estrangeiras. No caso dos Estados Unidos, os acordos bilaterais aplicáveis podem facilitar a validação, pela ANAC, de certificados de tipo emitidos pela FAA, sem implicar reconhecimento automático.
\end{itemize}

A escolha do grupo motopropulsor é diretamente condicionada por esses regulamentos. Os certificados de tipo da FAA associados às famílias O-540, IO-540, IO-720, IO-520 e TSIO-520, listadas na Tabela \ref{tab:motores_faixa}, podem facilitar o processo de homologação quanto ao atendimento aos requisitos de aeronavegabilidade brasileiros, conforme os acordos bilaterais aplicáveis. Motores não certificados (motores automotivos convertidos, motores 2T de uso industrial, motores BLDC de aplicação \textit{drone}) somente podem ser empregados sob regime de aeronave experimental (Subparte 21.191 do RBAC 21) com restrições operacionais e proibições de transporte oneroso.

\section{Conclusão e Combinações Recomendadas}

O objetivo deste trabalho foi comparar alternativas de motores, hélices e sistemas de monitoramento aplicáveis ao AGRO-VANT de asa fixa, tomando como referência aeronaves agrícolas tripuladas com MTOW entre 1{,}0 e 2{,}5 toneladas. O levantamento das plataformas de referência (Tabela \ref{tab:plataformas_referencia}), a comparação dos motores aplicáveis (Tabela \ref{tab:motores_faixa}) e a análise das curvas oficiais Lycoming (Seção \ref{sec:curvas_lycoming}) permitiram identificar alternativas de motorização, hélices e sistemas aviônicos para diferentes faixas de massa e perfis de missão. Os resultados indicam convergência para motores aeronáuticos de pistão certificados, de configuração boxer horizontalmente oposta, com potências entre \SI{235}{HP} e \SI{400}{HP}.

As combinações apresentadas na Tabela \ref{tab:recomendacoes} constituem recomendações preliminares, sujeitas ao dimensionamento detalhado da aeronave e à verificação da compatibilidade entre motor, hélice, estrutura e sistemas aviônicos.

\subsection{Cenários Operacionais Recomendados}

\begin{table}[H]
\centering
\caption{Recomendações preliminares de combinações propulsivas por cenário de missão}
...
\multicolumn{4}{l}{\footnotesize Elaboração do autor com base em \cite{lycomingmanual, lycoming_io720, continental_io520, embraer202, piper_pa36}.}
\end{tabularx}
\end{table}
...
\begin{enumerate}
    \item \textbf{Para a configuração base do AGRO-VANT (MTOW $\sim$\SI{1500}{kg}--\SI{2000}{kg}):} A recomendação primária é a adoção do conjunto Lycoming IO-540-K1J5 + Hartzell tripá HC-C3YR de velocidade constante (Cenário B), replicando o paradigma consolidado do EMB-202 Ipanema. Essa combinação reúne disponibilidade de assistência técnica no Brasil, suporte à operação com etanol e certificações que oferecem maior facilidade de atendimento aos requisitos aplicáveis. O peso seco do motor (\SI{200}{kg}) e seu envelope dimensional deverão ser verificados em conjunto com o dimensionamento estrutural e a análise de centro de gravidade do AGRO-VANT.
    ...
    \item \textbf{Para a aviônica:} Recomenda-se instrumentação digital integrada (EFIS Garmin TXi ou equivalente), com monitoramento CHT/EGT por cilindro, sensor de fluxo de combustível e DGPS de uso agrícola. A arquitetura de \textit{digital twin} para manutenção baseada em condição representa um caminho de modernização.
    
    \item \textbf{Sobre tração elétrica e hibridização:} Com as densidades de energia consideradas neste estudo, a tração puramente elétrica não atende ao balanço de massa da missão de referência na faixa de 1 a 2{,}5 t. A hibridização paralela representa uma linha de pesquisa a ser avaliada, mas ainda depende do dimensionamento completo do sistema e da evolução da certificação aeronáutica de motores elétricos e baterias.
\end{enumerate}

Como contribuição ao projeto AGRO-VANT, o estudo estabelece uma base técnica rastreável para a seleção preliminar do grupo motopropulsor e dos sistemas de monitoramento. Para a faixa central de \SI{1500}{kg} a \SI{2000}{kg}, o conjunto baseado no IO-540 constitui a alternativa de referência; para massas superiores, o IO-720 amplia a margem de potência; e, para operações recorrentes em elevada altitude-densidade, a opção turbocomprimida merece avaliação específica. A decisão final deverá ser apoiada por cálculos de desempenho da aeronave, compatibilidade vibratória entre motor e hélice, análise de massa e centro de gravidade, ensaios de bancada e validação dos requisitos de certificação.
...
\begin{thebibliography}{99}
    ...
    Continental Motors Inc. \textit{IO-520 and TSIO-520 Series Engine Specifications and Type Certificate Data Sheet}. Acessado em: 12 de maio de 2026. Disponível em: \url{https://drs.faa.gov/} e \url{https://continental.aero/}

    % ==== Aeronaves agrícolas de referência (faixa 1--2,5 t) ====

    \bibitem{embraer202}
    Embraer / Indústria Aeronáutica Neiva. \textit{EMB-200, EMB-201, EMB-202 e EMB-203 Ipanema: Especificações Técnicas}. Botucatu, Brasil. Acessado em: 12 de maio de 2026. Disponível em: \url{https://www.embraer.com/}

    \bibitem{embraer_1700}
    Embraer. \textit{Embraer celebra 1.700 unidades entregues do avião agrícola Ipanema}. Media Center, Botucatu, 19 de novembro de 2025. Versão atual EMB-203, com $\sim$60\% de participação no mercado nacional de pulverização. Disponível em: \url{https://www.embraer.com/media-center/pt/?detail=23507&mediatype=NEWS}

    \bibitem{piper_pa25}
    Piper Aircraft Corporation. \textit{PA-25 Pawnee Type Certificate Data Sheet and Pilot's Operating Handbook}. Variantes PA-25-150 (Lycoming O-320), PA-25-235 Pawnee B/C/D (Lycoming O-540-B2B5) e PA-25-260 (Lycoming O-540-E4B5). Acessado em: 12 de maio de 2026. TCDS disponível em: \url{https://drs.faa.gov/}

    \bibitem{cessna_188}
    Cessna Aircraft Company. \textit{Model 188 Series AgWagon, AgPickup, AgTruck and AgHusky: Type Certificate and Operating Handbook}. Wichita, KS. Acessado em: 12 de maio de 2026. TCDS disponível em: \url{https://drs.faa.gov/}

    \bibitem{piper_pa36}
    Piper Aircraft Corporation / WTA Incorporated. \textit{PA-36 Pawnee Brave Series: Type Certificate and Pilot's Operating Handbook}. Variantes Brave 300 (Lycoming IO-540-K1G5), Brave 375 e Brave 400 (Lycoming IO-720-D1CD). Acessado em: 12 de maio de 2026. TCDS disponível em: \url{https://drs.faa.gov/}
    ...
    \bibitem{anacrbac33}
    Agência Nacional de Aviação Civil (ANAC). \textit{RBAC nº 33: Requisitos de Aeronavegabilidade, Motores Aeronáuticos}. Disponível em: \url{https://www.anac.gov.br/assuntos/legislacao/legislacao-1/rbha-e-rbac/rbac/rbac-33}

    \bibitem{anacrbac35}
    Agência Nacional de Aviação Civil (ANAC). \textit{RBAC nº 35: Requisitos de Aeronavegabilidade, Hélices}. Disponível em: \url{https://www.anac.gov.br/assuntos/legislacao/legislacao-1/rbha-e-rbac/rbac/rbac-35}

    \bibitem{faa_tcds_1e4}
    Federal Aviation Administration (FAA). \textit{Type Certificate Data Sheet No. 1E4: Lycoming IO-540 Series Aircraft Engines}. Emitido em 5 de maio de 1960, múltiplas revisões. Disponível em: \url{https://drs.faa.gov/}

    \bibitem{faa_tcds_e295}
    Federal Aviation Administration (FAA). \textit{Type Certificate Data Sheet No. E-295: Lycoming O-540 Series Aircraft Engines}. Disponível em: \url{https://drs.faa.gov/}

    \bibitem{faa_tcds_p920}
    Federal Aviation Administration (FAA). \textit{Type Certificate Data Sheet No. P-920: Hartzell Propeller Inc., Compact Series Propellers (inclui a família HC-C2YK)}. Disponível em: \url{https://drs.faa.gov/}

    \bibitem{abnt9001}
    Associação Brasileira de Normas Técnicas (ABNT). \textit{NBR ISO 9001:2015: Sistemas de gestão da qualidade, requisitos}. Rio de Janeiro: ABNT, 2015.

    \bibitem{mapa_in02}
    Ministério da Agricultura, Pecuária e Abastecimento (MAPA). \textit{Instrução Normativa nº 2, de 3 de janeiro de 2008: Aprova as normas de trabalho da aviação agrícola} (com alterações da IN nº 37/2020), editada com base no Decreto-Lei nº 917/1969. Disponível em: \url{https://www.gov.br/agricultura/pt-br/assuntos/insumos-agropecuarios/aviacao-agricola/legislacao}

    \bibitem{abnt17146}
    Associação Brasileira de Normas Técnicas (ABNT). \textit{NBR 17146:2025: Sistemas de aeronaves não tripuladas, procedimentos operacionais}. Rio de Janeiro: ABNT, 2025. Alinhada à ISO 21384-3 (ISO/TC 20/SC 16).

    \bibitem{sindag_frota2025}
    Sindicato Nacional das Empresas de Aviação Agrícola (SINDAG). \textit{Análise da Frota Aeroagrícola Brasileira de Aviões e Helicópteros 2025}. Autor: Cláudio Júnior Oliveira Gomes. Frota registrada na ANAC: 2.866 aeronaves ao final de 2025 (crescimento de 5,25\% sobre 2024). Acessado em: maio de 2026.

    % ==== Referências adicionais (revisão de citações) ====

    \bibitem{battery_aviation_review}
    Pattanayak, T.; Mavris, D. (2025). \textit{Battery technology for sustainable aviation: a review of current trends and future prospects}. Applied Energy, 397, 126356. doi:10.1016/j.apenergy.2025.126356. Energy densities reported: jet fuel \SI{12000}{Wh/kg}; current Li-Ion up to \SI{330}{Wh/kg}; solid-state up to \SI{400}{Wh/kg}. Disponível em: \url{https://www.sciencedirect.com/science/article/pii/S0306261925010864}

    \bibitem{kartal2025}
    Kartal, M. R.; Ignatyev, D.; Zolotas, A. (2025). \textit{Comprehensive review of agriculture spraying UAVs challenges and advances: modelling and control}. The Aeronautical Journal, 129(1340), pp. 2859--2879. doi:10.1017/aer.2025.10031.

    \bibitem{sadeghi2024}
    Sadeghi, A.; Bellavista, P.; Song, W.; Yazdani-Asrami, M. (2024). \textit{Digital Twins for Condition and Fleet Monitoring of Aircraft: Toward More-Intelligent Electrified Aviation Systems}. IEEE Access, 12, pp. 99806--99832. doi:10.1109/ACCESS.2024.3371902.

    \bibitem{airinsight_dt}
    AirInsight. \textit{Aircraft Health Management Systems and Digital Twin Technology}. Discute sistemas de gerenciamento de saúde de aeronaves e digital twins em frotas modernas, incluindo a família Embraer E2. Disponível em: \url{https://airinsight.com/aircraft-health-management-systems-and-digital-twin-technology/}
    ...
    Hartzell Propeller Inc. \textit{Trailblazer Composite Propeller: Technical Description}. ``Materials [...] 20\% lighter than aluminum blade propellers.'' Acessado em maio de 2026. Disponível em: \url{https://hartzellprop.com/}
